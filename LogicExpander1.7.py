...
fontTitle = tkFont.Font(family="Lucida Grande", size=36)
fontLabels = tkFont.Font(family="Lucida Grande", size=16)
Welcome = Label(root, text='Welcome to the logic expander', font=fontTitle)
...
labelUD = ttk.Label(textbox, text="UD: ", font=fontLabels).grid(column=0,row=0)
labelUD2 = ttk.Label(textbox, text=UD, font=fontLabels).grid(column=0,row=1)
labelExp = ttk.Label(textbox, text='Expression:', font=fontLabels).grid(column=0,row=2)
labelExp2 = ttk.Label(textbox, text=expression, font=fontLabels).grid(column=0,row=3)

''' output display'''
output = Frame(root)
labelOUT = ttk.Label(output, text="output: ", font=fontLabels).grid(column=0,row=0)
...
def and_bt():
    ...
    expDisplay += u'\u2227'
    expression += temp
    labelExp2 = ttk.Label(textbox, text=expDisplay, font=fontLabels).grid(column=0,row=3)
    return None
...
def run():
    ...
    if inputChecker(expression):
        ...
        labelOUT2 = ttk.Label(output, text=out2, font=fontLabels).grid(column=0,row=1)
    else :
        labelOUT2 = ttk.Label(output, text='invalid input', font=fontLabels).grid(column=0,row=1)
    return None
...
def reset():
    ...
    for j in range(len(UD)):
        bsUD_bt()
        temp2 += " "
    labelExp2 = ttk.Label(textbox, text=temp*2, font=fontLabels).grid(column=0,row=3)
    labelUD2 = ttk.Label(textbox, text=temp2*2, font=fontLabels).grid(column=0,row=1)
    labelOUT2 = ttk.Label(output, text=temp*10, font=fontLabels).grid(column=0,row=1)
    return None
...
inputS.grid(column=1, row=1, columnspan=2)
inputS.bind("<Return>", enterS)

#entry for UD
inputUD = ttk.Entry(other)
inputUD.grid(column=1, row=3, columnspan=2)
...
LExp = Label(other, text="EXP:").grid(column=0, row=1)
LUD = Label(other, text="UD:").grid(column=0, row=3)
